(ns agentlang.lang.tools.nrepl.middleware.interruptible-eval
  ...
  (:require
    ...
    [agentlang.lang.tools.nrepl.core :refer [nrepl-eval-init]])
  (:import
  ...
  (let [explicit-ns (and ns (-> ns symbol find-ns))
    ...
    (if (and ns (not explicit-ns))
      ...
      (let [ctxcl (.getContextClassLoader (Thread/currentThread))
        ...
        (try
          (clojure.main/repl
           :eval (let [eval-fn (deref nrepl-eval-init)]
                   (fn [form]
