...
(defn operator-name [x]
  (case x
    :< 'agentlang.compiler.rule/lt
    :<= 'agentlang.compiler.rule/lteq
    :> 'agentlang.compiler.rule/gt
    :>= 'agentlang.compiler.rule/gteq
    :<> 'agentlang.compiler.rule/neq
    :like 'agentlang.compiler.rule/like
    :in 'agentlang.compiler.rule/in
    :between 'agentlang.compiler.rule/between
    (symbol (name x))))
...
(defn- accessor-expression
  ...
  (let [[c ref] (li/split-path n)]
    (let [parts (li/split-ref (or ref c))
          r (vec (rest parts))
          p (if (and c ref)
              [c (first parts)]
              (first parts))]
