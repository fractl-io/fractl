(ns agentlang.subs.kafka
  (:require [agentlang.util :as u]
            #?(:clj [agentlang.util.logger :as log]
               :cljs [agentlang.util.jslogger :as log])
            [agentlang.datafmt.json :as json]
            [agentlang.subs.internal :as si])
  #?(:clj
     (:import [java.time Duration]
              ...
              [agentlang.subs HandleRebalance])))
  ...
  #?(:clj
     (let [^Properties props (Properties.)]
       ...
       (.setProperty props ConsumerConfig/GROUP_ID_CONFIG
                     (or (:group-id config) "agentlang-consumers"))
...
(defn listen [client]
  #?(:clj
     (let [conn (si/connection client)
           ...
           config (:config conn)
           topic (or (:topic config) "agentlang-events")
