(ns fractl.inference.embeddings.generator
...
(defn generate-tool-for-event [event-name event-schema]
  (try
    (let [event-name (keyword event-name)
      ...
      (println "Generating tools complete for event!")
      fetch-maps)
    (catch Exception e
      (println "Generating events failed: " (.getMessage e))
      (throw e))))
  ...
  (try
    (let [entity-name (keyword entity-name)
          ...
          create-map {create-string-name {:description  (if (nil? (get-in entity-schema [:meta :comment]))
                                          ...
                                          :returns-many false}}]
      (println "Generating tools complete for entity!")
      (merge fetch-maps-formatted create-map))
    (catch Exception e
      (println "Generating fetch maps failed for entity: " (.getMessage e))
      (throw e))))
...
(defn generate-tool-for-data [tag type schema]
  (case tag
    ...
    :else (println (str "Don't know how to handle " tag))))
